...
- **Known limitation (not a bug introduced by i18n work):** strings rendered dynamically via `innerHTML` (e.g. the "N bookmarks in M categories" count line) are not re-translated live when the language selector changes, because `updateTranslations()` only re-scans `data-i18n` attributes. They render correctly on next tree re-render / reload.

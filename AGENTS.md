...
- **Tests (zero-dependency):** pure `src/core/` logic is covered by Node's built-in runner — run `node --test` (specs live in `test/*.test.js`). The i18n coverage guard is `node scripts/check-i18n.js`. Both run in CI (`.github/workflows/ci.yml`) on push/PR. There is still no lint config; "linting" is only `node --check` on individual JS files. Browser-only code (anything needing `DOMParser`, `localStorage`, or the DOM, e.g. `parseBookmarkHTML`) is not unit-tested here — verify it manually in the browser.
